...
public class AudioSystem {
  ...
  public static AudioInputStream getAudioInputStream(File file)
    ...
    if (file.getAbsolutePath().endsWith(".m4a")) {
      // This is an AAC file. Use the JAAD reader
      AudioFileReader reader = new AACAudioFileReader();
      try {
      ...
      // go through the standard SPI nonsense.
      for (int i = 0; i < providers.size(); i++) {
        AudioFileReader reader = (AudioFileReader) providers.get(i);
        try {
  ...
  private static List getProviders(Class providerClass) {
    ...
    final ServiceLoader loader = ServiceLoader.load(providerClass);
    final List rv = new ArrayList();
    System.err.println("Retrieved service loader for " + providerClass);

    for (Object o : loader) {
      System.err.println("Adding service provider: " + o);
      rv.add(o);
    }
    return rv;
  }
